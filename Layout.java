...
public class Layout {
	...
	protected LayoutManager getLayoutType(){
		...
		if (layoutType == FLOW) {
			...
			if (this.hasGaps()) {
				return new GridLayout( gridRows, gridColumns, gridHorizontalGap, gridVerticalGap);
			}
			else {
				return new GridLayout( gridRows, gridColumns);
			}
		}
		return null;
	}
	...
	public void type(){
		if (p.getCharacter() == Parser.SPACE){
			if (p.keywordMatch(LAYOUT)){
				...
				if (p.lookahead()=='F' && p.keywordMatch(FLOW)) {
					layoutType = FLOW;
				} else if (p.lookahead()=='G' && p.keywordMatch(GRID)) {
					 layoutType = GRID;
			 	} else {
					 p.errorMessage("Incorrect layout type.");
				}
			}
		}
	}
	...
	public void gridSizing(){
		Integer output = p.collectNumber(Parser.LEFT_PARENTHESIS, Parser.COMMA, false);
		...
		if (p.getCharacter() == Parser.SPACE || Character.isDigit( p.getCharacter())){
			...
			while ( !stop && p.getStatus() == Parser.state.PARSE ){
				...
				if ( letter == Parser.COMMA){
					if (gridColumns	== null ) {
						gridColumns = Integer.parseInt(output);
					} else if (gridHorizontalGap == null) {
						gridHorizontalGap = Integer.parseInt(output);
					} else {
						p.errorMessage("\")\" expected. \"" + letter + "\" found.");
					}
					...
					if (gridColumns	== null ) {
						...
						gridVerticalGap = Integer.parseInt(output);
					} else {
						p.errorMessage("\",\" expected. \"" + letter + "\" found.");
					}
					this.confirmColon();
